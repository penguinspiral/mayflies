...
The `_base` target provides foundational capabilities and configurations that other targets in the `red` host directory can inherit and extend. It serves as a "batteries included" baseline thereby accelerating development of new targets in a DRY fashion.

The `_base` target is buildable to ensure future development is validated before being consumed by another `red` target. Nevertheless it is recommended that usage of the `_base` target is implicit by means of inclusion from another `red` target.
...
* Leverages `live-config(7)` configurations:
  ...
  * Customisable hostname (MAYFLIES_HOSTNAME) & username (MAYFLIES_LIVE_USERNAME)
...
A minimal, functional example of this is the `red` host target: `motd`.
